...
├── menu.json           # ДАННЫЕ — iiko генерирует этот файл
└── images/
    ├── Espresso.jpg
    ├── Americano.jpg
    ├── фильтр кофе.jpg
...
### Принципы
1. **Всё из JSON** — iiko генерирует `menu.json`, фронтенд читает и рендерит.
2. **Изображения — маппинг в JS, не в JSON** — iiko не передаёт фото. Маппинг `itemImages` в `script.js` (константа + функция `getItemImage(itemName)`).
3. **Никаких хардкодов** — аддоны, табы, категории — всё динамически из JSON.

### Структура menu.json
```jsonc
{
  "addons": { /* ключ: определение аддона */ },
  "badges": { /* ключ: { icon, text } */ },
      ...
      "items": [
        {
          ...
          "priceDisplay": "от 320₽",     // для превью-карточки
          "sizes": ["M — 320₽", "L — 380₽"], // если есть размеры
          "addons": ["syrup", "milk"],       // ключи из addons
          "badge": "matcha"                  // опционально, ключ из badges
        }
      ]
    }
  ],
  "kitchen": [ /* та же структура, sizes опциональны */ ]
}
```

### iiko-генерация
Все ключи `menu.json`, кроме `tabs`, `addons`, `badges` — считаются секциями меню.
Код сам находит их: `Object.keys(json).filter(k => !['tabs','addons','badges'].includes(k))`.

---

## Система аддонов

Типы в `menu.json → addons`:
...
В `items[].addons[]` — массив ключей аддонов для этого товара.
...
### ✅ Готово
- Полностью data-driven архитектура (всё из JSON)
...
- **iiko генерирует только данные** — изображения в JS-маппинге
